...
from app.blueprints.contracts.routes import _apply_total_from_scope, _parse_price_mode
...
from app.utils.client_kinds import CLIENT_KIND_KEYS
from app.utils.client_queries import clients_to_json_results, search_clients_for_picker
from app.utils.logger import log

from .forms import ClientDeleteForm, ClientForm
...
def clients():
    ...
    client_kind_filter = request.args.get("client_kind", "")
    ...
    if client_kind_filter == "__none__":
        clients_query = clients_query.filter(Client.client_kind.is_(None))
    elif client_kind_filter in CLIENT_KIND_KEYS:
        clients_query = clients_query.filter(Client.client_kind == client_kind_filter)
    if sort_by == "full_name":
        ...
        clients_query = clients_query.order_by(
            Client.created_at.asc() if sort_order == "asc" else Client.created_at.desc()
        )
    else:
        clients_query = clients_query.order_by(
    ...
    filtered_clients = len(clients)

    client_requests = []
    if client_id:
        client_requests = (
            Request.query.filter_by(client_id=int(client_id))
    ...
    return render_template(
        ...
        client_kind_filter=client_kind_filter,
...
def add_client():
    ...
    if form.validate_on_submit():
        ...
        ck = (form.client_kind.data or "").strip() or None
        new_client = Client(
            full_name=full_name,
            phone=phone,
            address=address,
...
def edit_client(client_id):
    ...
    next_url = _safe_next_url(url_for("clients.client_detail", client_id=client_id))
    ...
    if form.validate_on_submit():
        ...
        client.client_kind = (form.client_kind.data or "").strip() or None
    ...
    return render_template("clients/edit.html", form=form, client=client, next_url=next_url)
...
@clients_bp.route("/api/search", methods=["GET"])
@login_required
def api_search_clients():
    ...
    try:
        ...
        limit = 20
    rows = search_clients_for_picker(q, kind, limit=limit)
    return jsonify({"results": clients_to_json_results(rows)})
...
def client_detail(client_id):
    ...
    materials = (
        db.session.query(Nomenclature.name, Nomenclature.price, Request.description)
        .select_from(Nomenclature)
        .join(Request, Nomenclature.id == Request.id)
...
def _parse_contract_extras():
    """Поля договора обслуживания: вид контрагента, дата заключения, периодичность, перечень."""
    counterparty_kind = (request.form.get("counterparty_kind") or "").strip() or None
    conclusion_date_str = (request.form.get("conclusion_date") or "").strip()
    term_note = (request.form.get("term_note") or "").strip() or None
    service_periodicity = (request.form.get("service_periodicity") or "").strip() or None
    equipment_scope = (request.form.get("equipment_scope") or "").strip() or None
    conclusion_date = None
    if conclusion_date_str:
        try:
            conclusion_date = datetime.strptime(conclusion_date_str, "%Y-%m-%d").date()
        except ValueError:
            pass
    return (
        counterparty_kind,
...
def _parse_emergency_contract_terms():
    cnt_raw = request.form.get("emergency_included_count")
    cost_raw = request.form.get("emergency_included_cost")
    try:
        cnt = (
            int(cnt_raw)
            if cnt_raw
...
def add_contract(client_id):
    ...
    try:
        total_price = float(total_price) if total_price not in (None, "") else 0
    except (ValueError, TypeError):
        total_price = 0
    ...
    try:
        ...
        return redirect(url_for("clients.client_detail", client_id=client_id) + "#tab-contracts")
    extras = _parse_contract_extras()
    emergency_terms = _parse_emergency_contract_terms()
    contract = Contract(
        client_id=client_id,
        ...
        emergency_included_count=emergency_terms[0],
        emergency_included_cost=emergency_terms[1],
        start_date=start_date,
        end_date=end_date,
        counterparty_kind=extras[0],
        conclusion_date=extras[1],
        term_note=extras[2],
        service_periodicity=extras[3],
        equipment_scope=extras[4],
    )
    if price_mode == "from_scope":
        _apply_total_from_scope(contract)
    db.session.add(contract)
    db.session.commit()
    log.info(f"Договор добавлен для клиента {client_id}")
    flash(
        "Договор добавлен. Заполните перечень позиций и синхронизируйте заявки при необходимости.",
        "success",
    )
    return redirect(url_for("contracts.view_contract", contract_id=contract.id, tab="scope"))


@clients_bp.route("/contract/<int:contract_id>/edit", methods=["POST"])
@login_required
def edit_contract(contract_id):
    ...
    contract.price_mode = price_mode
    contract.document_number = document_number
    contract.emergency_included_count = emergency_terms[0]
    contract.emergency_included_cost = emergency_terms[1]
    contract.start_date = start_date
    contract.end_date = end_date
    contract.counterparty_kind = extras[0]
    contract.conclusion_date = extras[1]
    contract.term_note = extras[2]
    contract.service_periodicity = extras[3]
    if price_mode == "from_scope":
        _apply_total_from_scope(contract)
    else:
        contract.total_price = total_price
    db.session.commit()
    log.info(f"Договор {contract_id} обновлён")
    flash("Договор сохранён", "success")
    return redirect(url_for("clients.client_detail", client_id=client_id) + "#tab-contracts")


@clients_bp.route("/api/contract/<int:contract_id>", methods=["GET"])
@login_required
def api_contract_json(contract_id):
    ...
    return jsonify(
        {
            ...
            if getattr(c, "emergency_included_count", None) is not None
            else "",
            "emergency_included_cost": float(c.emergency_included_cost)
            if getattr(c, "emergency_included_cost", None) is not None
            else "",
            "term_note": c.term_note or "",
            "service_periodicity": c.service_periodicity or "",
            "equipment_scope": c.equipment_scope or "",
            "total_price": c.total_price if c.total_price is not None else "",
...
def add_equipment_modal(client_id):
    ...
    if not serial_number:
        serial_number = (
            f"AUTO-{client_id}-{Equipment.query.filter_by(client_id=client_id).count() + 1}"
        )
    if not eq_type:
    ...
    if file.filename.endswith(".xlsx"):
        try:
            ...
            for _, row in df.iterrows():
                ...
                if existing_client:
                    flash(
                        f"Клиент с телефоном {phone} и адресом {address} уже существует. Пропускаем.",
                        "warning",
                    )
                    continue
