...
from app.models.all_models import Client, Equipment, Request, RequestStatus
...
def map():
    ...
    if form.validate_on_submit():
        ...
        clients_query = Client.query.filter(
            and_(Client.latitude.isnot(None), Client.longitude.isnot(None))
        )
        if client_search:
        ...
        if request_search:
            ...
            requests_query = requests_query.filter(
                (Request.request_number.ilike(like_r)) | (Request.description.ilike(like_r))
            )
        if client_search:
            like_c = f"%{client_search}%"
            requests_query = requests_query.join(
                Client, Client.id == Request.client_id, isouter=True
            ).filter(
...
def client_map(client_id):
    ...
    form = MapFilterForm()
    markers = (
        [{"lat": client.latitude, "lon": client.longitude, "popup": f"Клиент: {client.full_name}"}]
    ...
    markers += [
        {"lat": e.latitude, "lon": e.longitude, "popup": f"Оборудование: {e.type}"}
        for e in equipments
    ...
    return render_template(
        "map/map.html", client=client, form=form, markers=markers, yandex_api_key=yandex_api_key
    )
